<template>
  <div class="hotel-ticket-page">
    ...
    <div v-if="isLoading" class="hotel-ticket-page__state">
      Загружаем бронь...
    </div>
    <div
      v-else-if="errorMessage"
      class="hotel-ticket-page__state hotel-ticket-page__state--error"
    >
      {{ errorMessage }}
    </div>
    ...
    <section v-else-if="booking" class="hotel-ticket-page__card">
      <div class="hotel-ticket-page__header">
        ...
        <button
          ...
          class="hotel-ticket-page__cancel"
          type="button"
      ...
      <div class="hotel-ticket-page__layout">
        <aside class="hotel-ticket-page__qr-side">
          <a
            class="hotel-ticket-page__qr-box"
            :href="qrVerifyPath"
            target="_blank"
            rel="noopener"
          >
            <img
              class="hotel-ticket-page__qr-image"
              :src="qrImageUrl"
              alt="QR бронь"
            />
          </a>
          <p class="hotel-ticket-page__qr-caption">
            QR ведет на {{ qrVerifyPath }}
          </p>
          <span
            class="hotel-ticket-page__status"
            :class="`hotel-ticket-page__status--${bookingStatus}`"
            >{{ statusLabel }}</span
          >
        ...
        <div class="hotel-ticket-page__content">
          <div class="hotel-ticket-page__partner-line">
            <div class="hotel-ticket-page__partner">
              <img
                class="hotel-ticket-page__partner-avatar"
                :src="partnerAvatar"
                :alt="partnerTitle"
              />
              <span>{{ partnerTitle }}</span>
            </div>
            <span class="hotel-ticket-page__rating"
              >{{ starsLabel }} • {{ ratingLabel }}</span
            >
          </div>

          <div class="hotel-ticket-page__visual">
            <img
              class="hotel-ticket-page__image"
              :src="hotelImage"
              :alt="hotelName"
            />
          </div>
          ...
          <div class="hotel-ticket-page__summary">
            <div class="hotel-ticket-page__row">
              <span>Заезд</span><strong>{{ checkInLabel }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Выезд</span><strong>{{ checkOutLabel }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Гостей</span><strong>{{ guestsLabel }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Стоимость</span><strong>{{ totalLabel }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Имя</span><strong>{{ contactName }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Телефон</span><strong>{{ contactPhone }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Email</span><strong>{{ contactEmail }}</strong>
            </div>
            <div class="hotel-ticket-page__row">
              <span>Локация</span><strong>{{ hotelLocation }}</strong>
            </div>
          </div>
          ...
          <div
            ...
            class="hotel-ticket-page__review"
          >
            <template v-if="canReview">
              <h2 class="hotel-ticket-page__review-title">Оставить отзыв</h2>
              <p class="hotel-ticket-page__review-text">
                Бронь завершена, поэтому вы можете оценить этот отель.
              </p>

              <div
                class="hotel-ticket-page__review-stars"
                aria-label="Рейтинг отеля"
              >
                <button
                  v-for="star in 5"
                  :key="star"
                  ...
                  :class="{
                    'hotel-ticket-page__review-star--active':
              ...
              <p v-if="reviewError" class="hotel-ticket-page__review-error">
                {{ reviewError }}
              </p>
              <p v-if="reviewMessage" class="hotel-ticket-page__review-success">
                {{ reviewMessage }}
              </p>
          ...
          <div class="hotel-ticket-page__actions">
            <NuxtLink
              class="hotel-ticket-page__action hotel-ticket-page__action--primary"
              :to="hotelLink"
              >Перейти к отелю</NuxtLink
            >
            <NuxtLink
              class="hotel-ticket-page__action"
              :to="qrVerifyPath"
              target="_blank"
...
const requestStatusLabel = computed(() => {
  const value = normalizeString(
    booking.value?.requestStatus || booking.value?.status,
  );
...
const hotelName = computed(
  () => normalizeString(booking.value?.hotel?.name) || "Отель FlyAway",
);
const hotelImage = computed(
  () =>
    (Array.isArray(booking.value?.hotel?.images)
...
const hotelLocation = computed(
  () => normalizeString(booking.value?.hotel?.location) || "Локация уточняется",
);
...
const guestsLabel = computed(
  () => `${Math.max(1, Number(booking.value?.guests) || 1)}`,
);
const totalLabel = computed(() =>
  Number(booking.value?.total) > 0
    ? `${Number(booking.value.total).toLocaleString("ru-RU")} ₸`
    : "Уточняется",
);
const contactName = computed(() => normalizeString(booking.value?.name) || "—");
const contactPhone = computed(
  () => normalizeString(booking.value?.phone) || "—",
);
const contactEmail = computed(
  () => normalizeString(booking.value?.email) || "—",
);
const commentLabel = computed(
  () => normalizeString(booking.value?.comment) || "Без комментария",
);
const partnerTitle = computed(
  () =>
    normalizeString(
      booking.value?.hotel?.partner?.title || booking.value?.partner?.title,
    ) || "FlyAway Partner",
);
...
const ratingLabel = computed(() =>
  (Number(booking.value?.hotel?.rating) || 0).toFixed(1).replace(".", ","),
);
const starsLabel = computed(() => {
  const stars = Math.max(
    0,
    Math.round(Number(booking.value?.hotel?.rating) || 0),
  );
...
const hotelLink = computed(() =>
  booking.value?.hotel?._id ? `/hotels/${booking.value.hotel._id}` : "/hotels",
);
const qrVerifyPath = computed(() =>
  booking.value?._id
    ? `/qr/hotels/${booking.value._id}`
    : "/qr/hotels/not-found",
);
const qrVerifyUrl = computed(() =>
  typeof window !== "undefined"
    ? `${window.location.origin}${qrVerifyPath.value}`
    : qrVerifyPath.value,
);
...
.hotel-ticket-page__state,
.hotel-ticket-page__card {
  background: $white;
  border-radius: 24px;
...
.hotel-ticket-page__number {
  color: $surface-500;
  font-weight: 700;
  margin-bottom: 6px;
}
.hotel-ticket-page__title {
  color: $surface-900;
  font-size: 28px;
  font-weight: 900;
  line-height: 1.15;
}
.hotel-ticket-page__cancel {
  color: $orange-200;
  font-weight: 800;
}
.hotel-ticket-page__layout {
  display: grid;
  grid-template-columns: 240px minmax(0, 1fr);
  gap: 34px;
}
.hotel-ticket-page__qr-side {
  display: grid;
  gap: 16px;
  align-content: start;
}
.hotel-ticket-page__qr-box {
  display: flex;
  align-items: center;
...
.hotel-ticket-page__qr-image {
  width: 100%;
  max-width: 210px;
}
.hotel-ticket-page__qr-caption {
  color: $surface-500;
  font-size: 12px;
  overflow-wrap: anywhere;
  text-align: center;
}
.hotel-ticket-page__status {
  padding: 12px;
  min-height: 44px;
  border-radius: 999px;
  background: $red-500;
...
.hotel-ticket-page__request-meta {
  padding: 14px;
  border-radius: 16px;
...
.hotel-ticket-page__content {
  display: grid;
  gap: 20px;
}
.hotel-ticket-page__partner-line {
  display: flex;
  justify-content: space-between;
  gap: 12px;
  align-items: center;
}
.hotel-ticket-page__partner {
  display: flex;
  align-items: center;
  gap: 10px;
  color: $surface-500;
}
.hotel-ticket-page__partner-avatar {
  width: 32px;
  height: 32px;
  border-radius: 50%;
  object-fit: cover;
}
.hotel-ticket-page__rating {
  color: $surface-500;
  font-size: 13px;
}
.hotel-ticket-page__image {
  width: 100%;
  max-height: 320px;
  border-radius: 18px;
  object-fit: cover;
}
.hotel-ticket-page__section h2 {
  color: $surface-900;
  font-size: 17px;
  font-weight: 800;
  margin-bottom: 8px;
}
.hotel-ticket-page__section p {
  color: $surface-500;
  line-height: 1.5;
}
.hotel-ticket-page__summary {
  display: grid;
  gap: 8px;
}
.hotel-ticket-page__row {
  display: flex;
  justify-content: space-between;
  gap: 16px;
  color: $surface-500;
...
.hotel-ticket-page__actions {
  display: flex;
  flex-wrap: wrap;
  gap: 12px;
}
.hotel-ticket-page__action {
  min-height: 48px;
  padding: 0 24px;
  border-radius: 999px;
  display: inline-flex;
