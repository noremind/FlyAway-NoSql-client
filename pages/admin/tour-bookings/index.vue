<template>
  <section class="admin-bookings">
    ...
    <div v-else-if="!groups.length" class="admin-bookings__state">
      ...
      >
        ...
        <div class="admin-bookings__grid">
          ...
          <div class="admin-bookings__field">
            ...
            <span class="admin-bookings__value admin-bookings__value--accent"
              >{{ formatMoney(group.totalAmount) }} ₸</span
            >
          </div>
        </div>
      </NuxtLink>
...
useSeo({
  ...
  description: "Группировка бронирований туров FlyAway.",
});
...
const getTourImage = (tour) =>
  tour?.avatar ||
  (Array.isArray(tour?.images) ? tour.images.find(Boolean) : "") ||
  tourPlaceholder;

const statItems = computed(() => {
  const totalGroups = groups.value.length;
  const totalBookings = groups.value.reduce(
  ...
  const revenue = groups.value.reduce(
