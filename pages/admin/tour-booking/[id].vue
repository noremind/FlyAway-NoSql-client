<template>
  <section class="admin-booking-detail">
    ...
    <div v-if="isLoading" class="admin-booking-detail__state">
      Загружаем данные тура...
    </div>
    <div
      v-else-if="errorMessage"
      class="admin-booking-detail__state admin-booking-detail__state--error"
    >
      {{ errorMessage }}
    </div>
    ...
    <template v-else-if="group">
      <section class="admin-booking-detail__hero">
        <img
          class="admin-booking-detail__image"
          :src="tourImage"
          :alt="tourTitle"
        />
        <div class="admin-booking-detail__hero-content">
          ...
          <div class="admin-booking-detail__hero-meta">
            ...
            <span v-if="group?.tour?.departureCity">{{
              group.tour.departureCity
            }}</span>
      ...
      <div class="admin-booking-detail__stats">
        <article
          ...
          :key="item.label"
          class="admin-booking-detail__stat"
        >
      ...
      <section class="admin-booking-detail__panel">
        <div class="admin-booking-detail__panel-head">
          <div>
            ...
            <p class="admin-booking-detail__section-text">
              Здесь отображаются пользователи, которые купили этот тур.
            </p>
          </div>
          <UiSelect
            label="Статус"
        ...
        <div v-if="!bookings.length" class="admin-booking-detail__empty">
        ...
        <article
          ...
          :key="booking._id"
          class="admin-booking-detail__booking"
        >
          <div class="admin-booking-detail__booking-head">
            <div>
              <h3 class="admin-booking-detail__customer">
            ...
            <span
              class="admin-booking-detail__status"
              :class="`admin-booking-detail__status--${booking.status}`"
          ...
          <div class="admin-booking-detail__grid">
            ...
            <div class="admin-booking-detail__field">
              <span>QR</span><strong>{{ booking._id }}</strong>
            </div>
            <div
              class="admin-booking-detail__field admin-booking-detail__field--full"
            >
              <span>Билеты</span
              ><strong>{{
...
useSeo({
  title: "Детали бронирований тура",
  description: "Покупатели и бронирования конкретного тура.",
});
...
const tourTitle = computed(
  () => normalizeString(group.value?.tour?.title) || "Тур",
);
const tourDescription = computed(
  () =>
    normalizeString(group.value?.tour?.description) ||
    "Описание тура не указано.",
);
const partnerTitle = computed(
  () =>
    normalizeString(group.value?.tour?.partner?.title) || "Партнер не указан",
);
const tourImage = computed(
  () =>
    group.value?.tour?.avatar ||
    (Array.isArray(group.value?.tour?.images)
      ? group.value.tour.images.find(Boolean)
      : "") ||
    tourPlaceholder,
);
const bookings = computed(() =>
  Array.isArray(group.value?.bookings) ? group.value.bookings : [],
);
...
const getTicketsSummary = (ticketSelections) =>
  (Array.isArray(ticketSelections) ? ticketSelections : [])
...
.admin-booking-detail__state,
.admin-booking-detail__hero,
.admin-booking-detail__panel {
  padding: 22px;
  border-radius: 22px;
  background: $white;
...
.admin-booking-detail__image {
  width: 100%;
  height: 180px;
  border-radius: 18px;
  object-fit: cover;
}
.admin-booking-detail__eyebrow {
  color: $red-500;
  font-size: 12px;
  font-weight: 900;
...
.admin-booking-detail__text {
  margin-top: 10px;
  color: $surface-500;
  line-height: 1.55;
}
.admin-booking-detail__hero-meta {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
  margin-top: 14px;
...
.admin-booking-detail__stats {
  display: grid;
  grid-template-columns: repeat(4, 1fr);
  gap: 14px;
}
.admin-booking-detail__stat {
  padding: 18px;
  border-radius: 18px;
  background: $white;
  border: 1px solid rgba($red-500, 0.08);
}
.admin-booking-detail__stat-value {
  color: $red-500;
  font-size: 28px;
  font-weight: 900;
}
.admin-booking-detail__stat-label {
  margin-top: 8px;
  color: $surface-500;
  font-size: 13px;
}
...
.admin-booking-detail__panel-head {
  display: flex;
  justify-content: space-between;
  gap: 16px;
  align-items: flex-end;
}
.admin-booking-detail__section-title {
  color: $surface-900;
  font-size: 22px;
  font-weight: 900;
}
.admin-booking-detail__section-text,
.admin-booking-detail__muted {
  color: $surface-500;
  font-size: 14px;
  line-height: 1.45;
}
.admin-booking-detail__booking {
  display: grid;
  gap: 14px;
  padding: 18px;
  border-radius: 18px;
  border: 1px solid rgba($red-500, 0.08);
  background: rgba($surface-75, 0.8);
}
.admin-booking-detail__booking-head {
  display: flex;
  justify-content: space-between;
  gap: 12px;
  align-items: flex-start;
}
.admin-booking-detail__customer {
  color: $surface-900;
  font-size: 18px;
  font-weight: 900;
}
.admin-booking-detail__status {
  padding: 8px 12px;
  border-radius: 999px;
  color: $white;
  background: $red-500;
...
.admin-booking-detail__grid {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 12px;
}
.admin-booking-detail__field {
  display: grid;
  gap: 5px;
  padding: 12px;
  border-radius: 14px;
...
.admin-booking-detail__actions {
  max-width: 280px;
}
.admin-booking-detail__empty {
  padding: 22px;
  color: $surface-500;
  text-align: center;
}
